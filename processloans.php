...
<head>
  ...
  <title>UMCENGE UNDER PROCESS</title>
  ...
  <div class="card-body">
    ...
    <table id="memberstbl" class="display nowrap" style="width:100%">
        ...
        <tbody>
            ...
            $sql = "SELECT * FROM loans_application WHERE status = 'unprocessed' and member = ".$id." ORDER BY id DESC";
            ...
            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
                    ...
                    echo "<td>".$row['salarynet']."</td>";
                    echo "<td>".$row['add_income']."</td>";
                    echo "<td>".$row['add_income_value']."</td>";
                    echo "<td>".$row['total_income']."</td>";
                    echo "<td>".$row['rent']."</td>";
                    echo "<td>".$row['mortgage']."</td>";
                    echo "<td>".$row['utilities']."</td>";
                    echo "<td>".$row['school_fees']."</td>";
                    echo "<td>".$row['transport']."</td>";
                    echo "<td>".$row['other_loan']."</td>";
                    echo "<td>".$row['other_expenses']."</td>";
                    echo "<td>".$row['total_expenses']."</td>";
                    echo "<td>".$row['delivery']."</td>";
                    echo "<td> <button type='button' title='Process' data-id=".$row['id']." data-link='process.php?lid=".$row['id']."&mid=".$row['member']."' class='btn btn-success'><i class='bi bi-fast-forward'></i></button>
                    <button type='button' title='Decline' data-id=".$row['id']." data-link='editloan.php?mid=".$row['id']."' class='btn btn-danger'><i class='bi bi-x-square'></i></button>
                    </td>";
                    echo "</tr>";
                }
            }
