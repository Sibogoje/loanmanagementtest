...
if (isset($_POST['submit']) ) {
    ...
    $insert = $conn->prepare("INSERT INTO `members` (`name`, `surname`, `email`, `phone`, `postal`, `national_id`, `password`) VALUES (?, ?, ?, ?, ?, ?, ?)");
    $insert->bind_param("sssssss", $name, $surname, $email, $phone, $postal, $national_id, $password);
    $insert->execute();
    $insert->close();
...
<head>
  ...
  <title>UMCENGE ALL MEMBERS</title>
        ...
        <tbody>
            ...
            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
                    ...
                    echo "<td> <button type='button' title='More' data-id=".$row['id']." data-link='more.php?mid=".$row['id']."' class='btn btn-info'><i class='bi bi-info-circle'></i></button>
                    </td>";
                    echo "</tr>";
                }
            }
