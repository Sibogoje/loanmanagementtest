...
if (!file_exists($file_path)) {
    ...
    if (!in_array($extension, ['zip', 'pdf', 'docx', 'png', 'jpg', 'jpeg', 'doc'])) {
        ...
        if (move_uploaded_file($file, $file_path.'/'.$filename)) {
            $sql = "INSERT INTO loanfiles (`owner`, `name`, `dateupload`, `reason`) VALUES ('$owner','$filename', '$dates', '$reason')";
        ...
        <tbody>
            ...
            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
                    ...
                    echo "<td> <button type='submit' title='View' data-id='' data-link='uploads/".$row['owner']."/".$row['name']."'  class='btn btn-success'><i class='bi bi-eye'></i></button>
